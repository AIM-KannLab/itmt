...
input_annotation_file = 'data/csv_files/aomic.tsv'   
df = pd.read_csv(input_annotation_file, header=0,delimiter='\t')
input_path = "/mnt/kannlab_rfa/Anna/AOMIC/ds003097-download/"
save_to = 'data/t1_mris/aomic_reg/'
        ...
        if row['participant_id'] in filepath:
            for golden_file_path, age_values in age_ranges.items():
                if age_values['min_age'] <= age and age <= age_values['max_age']: 
                    print(age, input_path+filepath+"/anat/"+filepath+"_run-1_T1w.nii.gz", save_to, golden_file_path)
                    register_to_template(input_path+filepath+"/anat/"+filepath+"_run-1_T1w.nii.gz", save_to, golden_file_path, create_subfolder=False)
...
df.to_csv(path_or_buf= "data/Dataset_aomic.csv")
