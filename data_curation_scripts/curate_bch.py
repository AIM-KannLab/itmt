...
input_annotation_file = 'data/bch/matched.csv'   
df = pd.read_csv(input_annotation_file, header=0)
input_path = "data/bch/"
save_to = 'data/t1_mris/bch_reg/'
...
for idx in range(0, df.shape[0]):
    ...
    for filepath in os.listdir(input_path):
        ...
        if str(row['dicom_id']) in filepath:
...
df.to_csv(path_or_buf= "data/Dataset_bch.csv",header = ['AGE_M','SEX','SCAN_PATH','Filename','dataset'])
